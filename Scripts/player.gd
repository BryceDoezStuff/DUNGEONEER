...
@export var sprite_index: int = Global.Charachter
...
func _ready():
	...
	set_active_sprite()
	if Global.area: 
		position = position + Vector2(-768,500)
...
func _on_area_2d_3_body_shape_exited(body_rid, body, body_shape_index, local_shape_index):
	if body != null:
		if body.get_parent() != null and body.name == "walljump":
			gravity = 10
